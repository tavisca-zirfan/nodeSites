...
(function ($) {
    ...
    //For syntax refer https://github.com/elving/swag 
    ...
    Handlebars.registerHelper('forWhile', function(from,noOfIncr,incr, block) {
        var accum = '';
        while(noOfIncr>0){
            noOfIncr-=1;
            accum += block.fn(from);            
    ...
    Handlebars.registerHelper('markdown', function (value, options) {
        return markdown.toHTML(value || '');
    });

    Handlebars.registerHelper('getMoney', function (value, options) {
        if (_.isEmpty(value)) return '';
        var money = hfl.utils.parseMoney(value);
        var currency = BFHCurrenciesList[money.unit];
        return money.number + '' + ((_.isEmpty(currency) || _.isEmpty(currency.symbol)) ? money.unit : ' ' + currency.symbol);
    });

    Handlebars.registerHelper('currency', function (value, options) {
        if (_.isEmpty(value)) return '';
        var currency = BFHCurrenciesList[value];
        return _.isEmpty(currency) ? '' : currency.symbol;
    });

    Handlebars.registerHelper('getCurrency', function (value, options) {
        return window.hfl.utils.getCurrency(value);
    });

    Handlebars.registerHelper('getNumber', function (value, options) {
        if (!$.isNumeric(value)) return '0';
        var num = Number(value).format(2);
        return num;
    });

    // Register helper for moment js. For reference refer https://www.npmjs.com/package/handlebars-helper-moment
    Handlebars.registerHelper('moment', function (context, block) {
        if (context && context.hash) {
            block = $.extend(true, {}, _.clone(context));
            context = undefined;
        }
    ...
    Handlebars.registerHelper('pascal', function (val, block) {
        if (_.isEmpty(val) || !_.isString(val)) return '';
        return val.toPascalCase();
    });

    // Register duration helper for moment js. For reference refer https://www.npmjs.com/package/handlebars-helper-moment
        ...
        if (value) {
            if (_.isArray(value)) {
                value.forEach(function (x) {
                    $el.find('[value=\"' + x + '\"]').attr({ 'selected': 'selected' });
                    $el.find('[value=\"' + x.toLowerCase() + '\"]').attr({ 'selected': 'selected' });
                });
            } else {
                $el.find('[value=\"' + value + '\"]').attr({ 'selected': 'selected' });
                $el.find('[value=\"' + value.toLowerCase() + '\"]').attr({ 'selected': 'selected' });
            }
        } else {
